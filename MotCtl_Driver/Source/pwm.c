...
//! @FilePath: \F28335_MotCtl_Frameware\DSP28xx_MotCtl_Frameware\MotCtl_Driver\Source\pwm.c
//! @Author: Hex
//! @Date: 2023-08-20 21:25:26
//! @LastEditTime: 2024-06-05 22:41:04
//! @Description: 
...
void PWM_set_TB_Period(PWM_Handle pwmHandle, const uint16_t period)
{
...
void PWM_set_TB_SyncMode(PWM_Handle pwmHandle, const PWM_SyncMode_e syncMode)
{
    PWM_Obj *pwm = (PWM_Obj *)pwmHandle;
...
void PWM_set_TB_ClkDiv(PWM_Handle pwmHandle, const PWM_ClkDiv_e clkDiv)
{
    PWM_Obj *pwm = (PWM_Obj *)pwmHandle;

    pwm->TBCTL.bit.CLKDIV = clkDiv;

    return;
} // end of PWM_setClkDiv() function

void PWM_set_TB_HighSpeedClkDiv(PWM_Handle pwmHandle, const PWM_HspClkDiv_e clkDiv)
{
    PWM_Obj *pwm = (PWM_Obj *)pwmHandle;

    pwm->TBCTL.bit.HSPCLKDIV = clkDiv;

    return;
} // end of PWM_setHighSpeedClkDiv() function

void PWM_set_TB_PHSEN(PWM_Handle pwmHandle, const PWM_Enable_e Val)
{   
    PWM_Obj *pwm = (PWM_Obj *)pwmHandle;

    pwm->TBCTL.bit.PHSEN = Val;

    return;
}

void PWM_set_TB_RunMode(PWM_Handle pwmHandle, const PWM_RunMode_e runMode)
{
    PWM_Obj *pwm = (PWM_Obj *)pwmHandle;

    pwm->TBCTL.bit.FREE_SOFT = runMode;

    return;
} // end of PWM_setRunMode() function

void PWM_set_TB_CounterMode(PWM_Handle pwmHandle, const PWM_CounterMode_e counterMode)
{
    PWM_Obj *pwm = (PWM_Obj *)pwmHandle;

    pwm->TBCTL.bit.CTRMODE = counterMode;

    return;
} // end of PWM_setCounterMode() function

void PWM_set_TB_PhaseDir(PWM_Handle pwmHandle, const PWM_PhaseDir_e phaseDir)
{
    PWM_Obj *pwm = (PWM_Obj *)pwmHandle;

    // clear the bits
    pwm->TBCTL.bit.PHSDIR = phaseDir;

    return;
} // end of PWM_setPhaseDir() function

void PWM_set_TB_PeriodLoad(PWM_Handle pwmHandle, const PWM_PeriodLoad_e periodLoad)
{
    PWM_Obj *pwm = (PWM_Obj *)pwmHandle;

    pwm->TBCTL.bit.PRDLD = periodLoad;

    return;
} // end of PWM_setPeriodLoad() function

void PWM_set_TB_Phase(PWM_Handle pwmHandle, const uint16_t phase)
{
    PWM_Obj *pwm = (PWM_Obj *)pwmHandle;
...
// bool PWM_isCTR_reachMax(PWM_Handle pwmHandle)
// {
    
//     PWM_Obj *pwm = (PWM_Obj *)pwmHandle;

//     return (bool)pwm->TBSTS.bit.CTRMAX;
// }

// bool PWM_isSYN_occured(PWM_Handle pwmHandle)
// {
    
//     PWM_Obj *pwm = (PWM_Obj *)pwmHandle;

//     return (bool)pwm->TBSTS.bit.SYNCI;
// }

// bool PWM_isCount_UP (PWM_Handle pwmHandle)
// {
    
//     PWM_Obj *pwm = (PWM_Obj *)pwmHandle;

//     return (bool)pwm->TBSTS.bit.CTRDIR;
// }
