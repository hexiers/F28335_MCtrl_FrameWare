//! @FilePath: \DSP28xx_MotCtl_Frameware\MotCtl_Driver\Source\pie.c
//! @Author: Hex
...
void PIE_enableAdcSEQInt(PIE_Handle pieHandle, const int SEQNumber)
{
    PIE_Obj *pie = (PIE_Obj *)pieHandle;

    uint16_t setValue = (1 << SEQNumber);

    // set the value
    pie->PIEIER_PIEIFR[0].IER |= setValue;
                               ...
                               const PIE_IntVec_t vector)
{
    ...
    intPointer = (PIE_IntVec_t *)&(pie->ADCINT);
