...
//! @FilePath: \DSP28xx_MotCtl_Frameware\MotCtl_Driver\Include\sysctrl.h
//! @Author: Hex
//! @Date: 2023-08-20 21:25:26
//! @LastEditTime: 2023-08-26 09:19:05
...
extern SYSCTRL_Handle SYSCTRL_init(void *pMemory, const size_t numBytes);
...
extern void WDOG_clearCounter(SYSCTRL_Handle sysCtrl_Handle);

//! \brief     Disables the watchdog (WDOG) timer
//! \param[in] wdogHandle  The watchdog (WDOG) timer object handle
extern void WDOG_disable(SYSCTRL_Handle sysCtrl_Handle);

//! \brief     Disables the watchdog (WDOG) timer interrupt
//! \param[in] wdogHandle  The watchdog (WDOG) timer object handle
extern void WDOG_disableInt(SYSCTRL_Handle sysCtrl_Handle);

//! \brief     Disables the timer override
//! \param[in] wdogHandle  The watchdog (WDOG) timer object handle
extern void WDOG_disableOverRide(SYSCTRL_Handle sysCtrl_Handle);

//! \brief     Enables the watchdog (WDOG) timer
//! \param[in] wdogHandle  The watchdog (WDOG) timer object handle
extern void WDOG_enable(SYSCTRL_Handle sysCtrl_Handle);

//! \brief     Enables the watchdog (WDOG) timer interrupt
//! \param[in] wdogHandle  The watchdog (WDOG) timer object handle
extern void WDOG_enableInt(SYSCTRL_Handle sysCtrl_Handle);

//! \brief     Enables the watchdog (WDOG) timer override
//! \param[in] wdogHandle  The watchdog (WDOG) timer object handle
extern void WDOG_enableOverRide(SYSCTRL_Handle sysCtrl_Handle);

//! \brief     Gets the watchdog (WDOG) interrupt status
//! \param[in] wdogHandle  The watchdog (WDOG) timer object handle
//! \return    The interrupt status
extern WDOG_IntStatus_e WDOG_getIntStatus(SYSCTRL_Handle sysCtrl_Handle);
...
extern SYSCTRL_Handle WDOG_init(void *pMemory, const size_t numBytes);

//! \brief     Sets the watchdog (WDOG) counter
//! \param[in] wdogHandle  The watchdog (WDOG) timer object handle
//! \param[in] count       The count
extern void WDOG_setCount(SYSCTRL_Handle sysCtrl_Handle, const uint_least8_t count);

//! \brief     Sets the watchdog (WDOG) timer clock prescaler
//! \param[in] wdogHandle  The watchdog (WDOG) timer object handle
//! \param[in] preScaler   The prescaler
extern void WDOG_setPreScaler(SYSCTRL_Handle sysCtrl_Handle, const WDOG_PreScaler_e preScaler);
