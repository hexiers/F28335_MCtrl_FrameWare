...
void HAL_setupPeripheralClocks(HAL_Handle handle)
{
  ...
  Sys_enableAdcClock(obj->pllHandle);

  ADC_cal(); // 执行ADC校正代码

  Sys_enableScibClock(obj->pllHandle);

  Sys_disableTbClockSync(obj->pllHandle);   //关闭Tb时钟
  Sys_enablePwmClock(obj->pllHandle, PWM_Number_1);
  Sys_enablePwmClock(obj->pllHandle, PWM_Number_2);
  Sys_enablePwmClock(obj->pllHandle, PWM_Number_3);
  Sys_enableTbClockSync(obj->pllHandle);

  Sys_enableEqep1Clock(obj->pllHandle);

  Sys_enableCpuTimerClock(obj->pllHandle, CLK_CpuTimerNumber_0);

  Sys_enableGpioInputClock(obj->pllHandle);

  Sys_enableClkIn(obj->pllHandle);
