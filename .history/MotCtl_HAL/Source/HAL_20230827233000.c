...
void HAL_setupAdc(HAL_Handle handle)
{
  ...
  ADC_reset(obj->adcHandle);

  for(uint16_t i = 0;i<100;i++)	NOP;	

  ADC_setSuspendMode(obj->adcHandle,ADC_SUS_StopImmediately);

  ADC_setAcqWindowSize(obj->adcHandle,ADC_SOC_PulseClk_3);

	ADC_setClockPrescale(obj->adcHandle,ADC_HSPCLK_Div_1 ,ADC_CLKPS_Div_10);

  ADC_setRunMode(obj->adcHandle,ADC_Continuous_mode);

	ADC_setSeqMode(obj->adcHandle,ADC_SeqMode_DualSeq,ADC_SeqOvrd_disabled);

  ADC_setRefSource(obj->adcHandle,ADC_ExtRef_1_5V);

	for(i=0;i<100;i++)	NOP;

	ADC_setADCBGRFDN(obj->adcHandle,);
